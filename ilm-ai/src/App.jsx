    ...
    topics: [
      {
        ...
        notes:
          "Basic concepts of probability, sample spaces, events. Bayes' theorem is crucial here.",
      },
      {
        id: "topic1_2",
        name: "Bayesian Networks",
        progress: 60,
        files: [{ name: "Chapter2_BN.pdf", url: "#" }],
  ...
  cs101_topic1_1: [
    {
      ...
      type: "recall",
      explanation:
        "Conditional probability P(A|B) is the likelihood of event A occurring given that B is true. Formula: P(A|B) = P(A ∩ B) / P(B). It is foundational for Bayes' Theorem.",
    },
    {
      id: "q2",
      ...
      type: "test",
      explanation:
        "Bayes' Theorem describes the probability of an event based on prior knowledge of conditions that might be related to the event. Formula: P(A|B) = [P(B|A) * P(A)] / P(B).",
    },
  ],
  cs101_topic1_2: [
    {
      ...
      type: "test",
      explanation:
        "D-separation (direction-dependent separation) is a graphical criterion used to determine whether a set of nodes X is independent of another set Y, given a third set Z in a Bayesian network.",
    },
  ],
};
...
const ProgressBar = ({ progress, size = "h-2.5", color = "bg-blue-600" }) => (
  <div className={`w-full bg-gray-200 rounded-full ${size} dark:bg-gray-700`}>
    <div
      className={`${color} ${size} rounded-full`}
...
const Card = ({ children, className = "" }) => (
  <div
    className={`bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 ${className}`}
  >
    {children}
  </div>
);
...
const Button = ({
  ...
  const baseStyles =
    "font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-opacity-50 transition-all duration-150 ease-in-out flex items-center justify-center gap-2";
  const variantStyles = {
    primary: `bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500 ${
      disabled ? "bg-blue-300 hover:bg-blue-300 cursor-not-allowed" : ""
    }`,
    secondary: `bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 focus:ring-gray-400 ${
...
function App() {
  ...
  const handleAddCourse = (newCourseData) => {
    const newCourse = {
      ...
      topics: [],
      ...newCourseData,
      color: ["bg-purple-500", "bg-pink-500", "bg-indigo-500"][
        courses.length % 3
      ], // Cycle through some colors
      icon: [
        <BookOpen className="w-8 h-8 text-purple-100" />,
        <Target className="w-8 h-8 text-pink-100" />,
  ...
  const startQuiz = (type, courseId, topicId = null) => {
    ...
    if (topicId) {
      ...
      if (type === "flash")
        questions = questions.filter((q) => q.type === "recall");
    } else {
      // Course-level personalized quiz
    ...
    if (questions.length === 0) {
      ...
      questions.push({
        id: "q_default",
        text: "No questions available for this section yet. Try another one!",
        explanation:
          "Please add questions to the mockQuizQuestions object for this topic/course.",
      });
    }
  ...
  const handlePersonalizeNotes = (topic) => {
    setFeedbackMessage(
      `Personalized notes for "${topic.name}" are being generated based on your weakest points... (This is a simulation)`
    );
    ...
    setTimeout(() => {
      ...
      setFeedbackMessage(
        `Personalized notes for "${topic.name}" have been updated!`
      );
    }, 2000);
  };
      ...
      case "dashboard":
        return (
          <DashboardView
            ...
            onAddCourseClick={() => navigateTo("addCourse")}
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-inter p-4 sm:p-6 lg:p-8">
      ...
      <footer className="text-center mt-12 text-sm text-gray-500 dark:text-gray-400">
        <p>
          &copy; {new Date().getFullYear()} LearnSmart AI. Personalized learning
          journey.
        </p>
      </footer>
    </div>
...
const DashboardView = ({
  ...
  <div>
    <div className="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
      ...
      <Button onClick={onAddCourseClick} icon={PlusCircle} size="md">
        Add New Course
      </Button>
    </div>
    ...
    {courses.length === 0 && !searchTerm && (
      <Card className="text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4">
          Click "Add New Course" to get started with your learning journey.
        </p>
      </Card>
    )}
    ...
    {courses.length === 0 && searchTerm && (
      <Card className="text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Try a different search term or add a new course.
        </p>
      </Card>
    )}
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {courses.map((course) => (
        <Card
          key={course.id}
          className={`hover:shadow-xl transition-shadow cursor-pointer ${course.color} text-white`}
        >
          <div
            onClick={() => onNavigate("courseView", { courseId: course.id })}
          >
            ...
            <p className="text-sm opacity-90 mb-4 h-10 overflow-hidden">
              {course.description}
            </p>
          ...
          <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md">
            <div className="space-y-1 text-center">
              ...
              <div className="flex text-sm text-gray-600 dark:text-gray-400">
                <label
                  htmlFor="file-upload"
                  className="relative cursor-pointer bg-white dark:bg-gray-700 rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500 dark:ring-offset-gray-800"
                >
  ...
  <div className="space-y-8">
    <Card>
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
        <CircularProgressBar
          ...
          size={160}
          strokeWidth={12}
          color={`text-${course.color.split("-")[1]}-600 dark:text-${
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
            <Button
              onClick={() => onStartQuiz("personalized", course.id)}
              icon={Target}
            ...
            <Button
              onClick={() =>
                alert("Full progress view (knowledge graph) coming soon!")
              }
              variant="secondary"
              icon={BarChart3}
            >
            ...
            <Button
              onClick={() => alert("Quiz history coming soon!")}
              variant="secondary"
              icon={ListChecks}
            >
              Quiz History
    ...
    <div>
      ...
      {course.topics.length === 0 ? (
        <Card>
          <p className="text-gray-600 dark:text-gray-400">
            No topics added to this course yet. You can add topics by editing
            the course (feature coming soon!).
          </p>
        </Card>
      ) : (
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 truncate">
                    {topic.notes || "No notes for this topic yet."}
                  </p>
                </div>
                <div className="w-full sm:w-auto flex items-center gap-4 mt-3 sm:mt-0">
                  <div className="w-24 text-right">
                    ...
                    <ProgressBar
                      ...
                      size="h-2"
                      color={`bg-${course.color.split("-")[1]}-500`}
                    />
                  </div>
                  <Button
                    onClick={() =>
  ...
  <div className="space-y-8">
    <Card>
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
        ...
        />
        <div className="flex-1 text-center md:text-left">
          <p className="text-sm text-blue-500 dark:text-blue-400 font-medium mb-1">
            {courseName}
          </p>
          <h2 className="text-3xl font-bold mb-3">{topic.name}</h2>
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
            <Button
              onClick={() => onStartQuiz("flash", selectedCourse.id, topic.id)}
              icon={Lightbulb}
            ...
            <Button
              onClick={() => onStartQuiz("test", selectedCourse.id, topic.id)}
              icon={HelpCircle}
            ...
            <Button
              onClick={() => alert("Topic-specific quiz history coming soon!")}
              variant="secondary"
              icon={ListChecks}
            >
              Quiz History
...
const QuizView = ({
  ...
  const getQuizTitle = () => {
    ...
    if (quizType === "personalized") title = `Personalized Quiz: ${courseName}`;
      ...
      title = `Flash Quiz: ${topicName || courseName}`;
    else if (quizType === "test")
      title = `Test Quiz: ${topicName || courseName}`;
    return title;
  };
      ...
      {showExplanation && (
        <div className="space-y-4">
          <div>
            ...
            <p className="p-3 bg-gray-100 dark:bg-gray-700 rounded-md">
